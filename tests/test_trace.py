...
WINDOWS_COMPLETE = """\
Tracing route to 10.20.30.7 over a maximum of 3 hops:
  1     1 ms     1 ms     1 ms  10.20.30.1
  2     *        *        *     Request timed out.
  3     4 ms     4 ms     4 ms  10.20.30.7
"""
LINUX_ALTERNATE = """\
traceroute to 10.20.30.7 (10.20.30.7), 3 hops max
 1  10.20.30.1  1.0 ms  1.1 ms
 2  10.20.30.2  2.0 ms  10.20.30.3  2.1 ms
 3  10.20.30.7  4.0 ms
"""
...
class TraceParserTests(unittest.TestCase):
    def test_complete_unanswered_and_alternate_hops_remain_raw_evidence(self):
        ...
        self.assertEqual(windows[-1].addresses, ("10.20.30.7",))
        linux = parse_linux_trace(LINUX_ALTERNATE)
        self.assertEqual(linux[1].addresses, ("10.20.30.2", "10.20.30.3"))
        self.assertIn("10.20.30.3", linux[1].raw)

    def test_fixed_argv_uses_only_normalized_addresses_and_bounds(self):
        self.assertEqual(windows_trace_argv("10.20.30.7", max_hops=3, timeout_s=0.2), ("tracert.exe", "-d", "-h", "3", "-w", "200", "10.20.30.7"))
        self.assertEqual(linux_trace_argv("10.20.30.7", max_hops=3, timeout_s=0.2), ("traceroute", "-n", "-m", "3", "-w", "0.2", "10.20.30.7"))
        with self.assertRaises(ValueError):
            linux_trace_argv("10.20.30.7;whoami", max_hops=3, timeout_s=0.2)
...
class TraceServiceTests(unittest.IsolatedAsyncioTestCase):
    async def test_scope_and_authorization_fail_before_native_command(self):
        with self.assertRaises(PolicyError):
            TraceRequest("203.0.113.7", "10.20.30.0/24", authorized=True)
        request = TraceRequest("10.20.30.7", "10.20.30.0/24", authorized=False)
        with self.assertRaises(PolicyError):
    ...
    async def test_repeats_keep_alternate_paths_without_a_route_claim(self):
        request = TraceRequest("10.20.30.7", "10.20.30.0/24", max_hops=3, repeats=2, timeout_s=0.2, authorized=True)
