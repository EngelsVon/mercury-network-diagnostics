...
- [ ] 支持 Windows 和 Ubuntu 的普通用户模式，并对需要原始套接字、抓包或系统命令权限的能力进行降级说明；macOS 不属于 v1 发布目标。
...
- **可移植性**: Windows 与 Ubuntu 均须有可用的非特权基线，特权能力必须可选降级；macOS 可报告 unsupported，但不作为 v1 承诺。
