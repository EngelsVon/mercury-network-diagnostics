...
- [ ] **SAFE-02**: 系统对每个任务强制执行主机数、端口数、尝试/包数、发送字节、全局与每目标速率、并发数、持续时间、事件数和输出大小的不可绕过绝对上限；绝对上限至少允许单主机 1–65535 TCP 检查，但不允许多维无界笛卡尔积。
...
- [ ] **WEB-02**: 用户可从 WebUI 提交 basic 诊断、授权 discovery 和 route 任务，轮询查看进度、取消并打开结构化结果。
